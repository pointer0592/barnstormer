      ...
      {isNotIframe ? (
        <div className={`sidebar ${isOpen ? '' : 'sidebar-collapsed'}`}>
          ...
          <label className="sidebar-search">
            <BsInput value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search" />
            <button type="button" className="sidebar-search__button" onClick={() => setSearch('')}>
              {search ? <BsIconCloseSm className="sidebar-search__button-icon" /> : undefined}
            </button>
          </label>
              ...
              <Fragment key={group}>
                ...
                {groups[group].visible && (
                  <li className="flex flex-col select-none">
                    ...
                    >
                      {group}
                      <BsIconExpandMore className={`${groups[group].open ? 'rotate-180' : ''}`} />
                    </button>
                    {groups[group].open ? (
                      <ul>
