import { BsModal, BsButton } from '@barnstormer/react';
...
function Example() {
  ...
  return (
    <ComponentExample controls={{ state, controls }}>
      <BsButton onClick={() => state.set((currentState) => ({ ...currentState, open: !currentState.open }))}>
        Open Modal
      </BsButton>

      <BsModal {...state.get} onClose={() => state.set({ open: false })} className="max-w-sm">
        {state.get.children || (
          <>
            <p className="mb-2">
