...
// #region source
import { BsDrawer, type BsDrawerProps, BsButton } from '@barnstormer/react';
import { ChangeEventHandler, useState } from 'react';
import classNames from 'classnames';

export default function DrawerDemo() {
  const [open, setOpen] = useState(false);
  const [placementState, setPlacement] = useState<BsDrawerProps['placement']>('left');

  const changePlacement: ChangeEventHandler<HTMLSelectElement> = (event) => {
    setPlacement((event.target as HTMLSelectElement).value as BsDrawerProps['placement']);
  };
