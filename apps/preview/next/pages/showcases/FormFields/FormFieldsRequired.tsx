    ...
    <div className="px-4">
      ...
      <form onSubmit={sendForm}>
        <label className="flex justify-between cursor-pointer mb-4 gap-2">
          ...
          <BsSwitch checked={personalInformation} onChange={() => setPersonalInformation(!personalInformation)} />
        </label>
        <label>
          ...
          >
            {dropdownOptions.map((option) => (
              <BsListItem
                ...
                slotSuffix={option.value === selectedOption?.value && <BsIconCheck className="text-primary-700" />}
              >
                {option.label}
              </BsListItem>
            ))}
        ...
        <fieldset role="radiogroup">
          ...
          {radioOptions.map(({ label, name, value }) => (
            ...
            >
              <BsRadio value={value} name={name} required onChange={() => setPushNotifications(value)} />
        ...
        <div className="flex gap-x-4 md:justify-end mt-6">
          <BsButton variant="secondary" className="flex-grow md:flex-grow-0">
            Clear all
          </BsButton>
          <BsButton type="submit" className="flex-grow md:flex-grow-0">
            Submit
          </BsButton>
        </div>
      </form>
