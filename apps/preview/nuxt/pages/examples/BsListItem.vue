<template>
  <ComponentExample :controls-attrs="controlsAttrs">
    <BsListItem class="max-w-sm" v-bind="state" @click="state.selected = !state.selected">
...
import { BsListItem, BsListItemSize, BsCounter, BsIconChevronRight, BsIconTune } from '@barnstormer/vue';
...
const prefixSlotOptions = createControlsOptions({
  none: undefined,
  'Tune icon': h(BsIconTune),
});
const suffixSlotOptions = createControlsOptions({
  none: undefined,
  'Chevron right icon': h(BsIconChevronRight),
});
const { controlsAttrs, state } = prepareControls(
  ...
  ],
  {
    ...
    size: ref<BsListItemSize>(BsListItemSize.base),
    counter: ref(123),
