<template>
  <aside class="w-full md:max-w-[376px]">
    ...
    <div class="px-2">
      <BsSelect v-model="sortModel" aria-label="Sort by">
    ...
    <ul>
      ...
      <li v-for="{ id: filterDataId, type, summary, details }, index in filtersData" :key="filterDataId">
        <BsAccordionItem v-model="opened[index]">
          <template #summary>
            <div class="flex justify-between p-2 mb-2">
              <p class="p-2 font-medium typography-headline-5">{{ summary }}</p>
              <BsIconChevronLeft :class="opened[index] ? 'rotate-90' : '-rotate-90'" />
            </div>
          </template>
          <ul v-if="type === 'size'" class="grid grid-cols-5 gap-2">
            <li v-for="{ id, value, counter, label } in details" :key="id">
              <BsChip v-model="selectedFilters" class="w-full" size="sm" :input-props="{ value, disabled: !counter }">
                {{ label }}
              </BsChip>
            </li>
          </ul>
          <template v-if="type === 'category'">
            <ul class="mt-2 mb-6">
              <li>
                <BsListItem size="sm" tag="button" type="button">
                  <div class="flex items-center">
                    <BsIconArrowBack size="sm" class="text-neutral-500 mr-3" />
              ...
              <li v-for="({ id, link, label, counter }, categoryIndex) in details" :key="id">
                ...
                >
                  <template #suffix>
                    <BsIconCheck v-if="categoryIndex === 0" size="xs" class="text-primary-700" />
                  </template>
                  <span class="flex items-center">
                    {{ label }}
                    <BsCounter class="ml-2 typography-text-sm">{{ counter }}</BsCounter>
                  </span>
                </BsListItem>
              </li>
            </ul>
            ...
            >
              <template #prefix>
                ...
                <span
                  ...
                  ><BsThumbnail size="sm" :class="value"
                /></span>
              </template>
              <p>
    ...
    <div class="flex justify-between">
      <BsButton variant="secondary" class="w-full mr-3" @click="handleClearFilters()"> Clear all filters </BsButton>
      <BsButton class="w-full">Show products</BsButton>
    </div>
  </aside>
</template>
