<template>
  <div>
    <div class="md:shadow-lg md:rounded-md md:border md:border-neutral-100">
      ...
      <div class="px-4 pb-4 mt-3 md:px-6 md:pb-6 md:mt-0">
        ...
        <div v-if="promoCode" class="flex items-center mb-5 py-5 border-y border-neutral-200">
          <p>PromoCode</p>
          <BsButton size="sm" variant="tertiary" class="ml-auto mr-2" @click="removePromoCode">Remove</BsButton>
        ...
        <form v-else class="flex gap-x-2 py-4 border-y border-neutral-200 mb-4" @submit.prevent="checkPromoCode">
          <BsInput v-model="inputValue" placeholder="Enter Promo Code" wrapper-class="grow" />
          <BsButton type="submit" variant="secondary">Apply</BsButton>
        </form>
        ...
        <BsButton size="lg" class="w-full"> Place Order And Pay </BsButton>
        <div class="typography-text-sm mt-4 text-center">
          By placing my order, you agree to our <BsLink href="#">Terms and Conditions</BsLink> and our
          <BsLink href="#">Privacy Policy.</BsLink>
        </div>
      </div>
    </div>
    <div class="absolute top-0 right-0 mx-2 mt-2 sm:mr-6">
      ...
      >
        <BsIconCheckCircle class="mr-2 my-2 text-positive-700" />
        ...
        >
          <BsIconClose class="hidden md:block" />
          <BsIconClose size="sm" class="md:hidden block" />
        </button>
      </div>
      <div
...
import { BsButton, BsInput, BsLink, BsIconClose, BsIconCheckCircle } from '@barnstormer/vue';
