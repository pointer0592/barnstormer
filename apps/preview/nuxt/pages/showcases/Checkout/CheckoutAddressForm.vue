<template>
  <form class="p-4 flex gap-4 flex-wrap text-neutral-900" @submit.prevent="onSubmit">
    ...
    <label class="w-full flex flex-col gap-0.5 md:w-[120px]">
      ...
      <BsInput name="zipCode" placeholder="eg. 12345" autocomplete="postal-code" required />
    </label>

    <label class="w-full flex items-center gap-2">
      <BsCheckbox name="useAsShippingAddress" />
      Use as shipping address
    </label>

    <div class="w-full flex gap-4 mt-4 md:mt-0 md:justify-end">
      <BsButton type="reset" variant="secondary" class="w-full md:w-auto">Clear all</BsButton>
      <BsButton type="submit" class="w-full md:w-auto">Save</BsButton>
    </div>
  </form>
</template>
...
import { ref } from 'vue';
import { BsSelect, BsInput, BsCheckbox, BsButton } from '@barnstormer/vue';
