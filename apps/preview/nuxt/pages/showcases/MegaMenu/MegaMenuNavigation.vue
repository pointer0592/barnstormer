            ...
            >
              <template v-for="node in activeMenu.children" :key="node.key">
                <template v-if="node.isLeaf">
                  <BsListItem tag="a" size="sm" :href="node.value.link" class="typography-text-sm mb-2">
                    {{ node.value.label }}
                  </BsListItem>
                ...
                <div v-else>
                  ...
                  <ul class="mt-2">
                    <li v-for="child in node.children" :key="child.key">
                      <BsListItem tag="a" size="sm" :href="child.value.link" class="typography-text-sm py-1.5">
                        {{ child.value.label }}
                      </BsListItem>
                    </li>
                  </ul>
      ...
      >
        <nav>
          <div class="flex items-center justify-between p-4 border-b border-b-neutral-200 border-b-solid">
            ...
            <BsButton variant="tertiary" square aria-label="Close menu" class="ml-2" @click="close()">
              <BsIconClose class="text-neutral-500" />
            </BsButton>
          </div>
          <ul class="mt-2 mb-6">
            <li v-if="activeMenu.key !== 'root'">
              ...
              >
                <div class="flex items-center">
                  <BsIconArrowBack class="text-neutral-500" />
            ...
            <template v-for="node in activeMenu.children" :key="node.value.label">
              <li v-if="node.isLeaf">
                <BsListItem size="lg" tag="a" :href="node.value.link" class="first-of-type:mt-2">
                  <div class="flex items-center">
                    <p class="text-left">{{ node.value.label }}</p>
                    <BsCounter class="ml-2">{{ node.value.counter }}</BsCounter>
                  </div>
                </BsListItem>
              </li>
              <li v-else>
                <BsListItem size="lg" tag="button" type="button" @click="goNext(node.key)">
                  <div class="flex justify-between items-center">
                    <div class="flex items-center">
                      <p class="text-left">{{ node.value.label }}</p>
                      <BsCounter class="ml-2">{{ node.value.counter }}</BsCounter>
                    </div>
                    <BsIconChevronRight class="text-neutral-500" />
                  </div>
                </BsListItem>
              </li>
            </template>
