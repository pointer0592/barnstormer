...
import { type BsRatingButtonProps } from '@barnstormer/react';
import { mount, useComponent } from '../../utils/mount';

const { vue: BsRatingButtonVue, react: BsRatingButtonReact } = useComponent('BsRatingButton');
import BsRatingButtonBaseObject from './BsRatingButton.PageObject';
import { isReact } from '../../utils/utils';

describe('BsRatingButton', () => {
  const page = () => new BsRatingButtonBaseObject('ratingbutton');

  const initializeComponent = (props: BsRatingButtonProps = {}) => {
    const { value, onChange, max, name, disabled, size, getLabelText, children } = props;
    return mount({
      vue: {
        component: BsRatingButtonVue,
        props: {
          modelValue: value,
  ...
  describe('when prop size is set to ', () => {
    Object.values(BsRatingButtonSize).forEach((size) => {
      describe(`${size}`, () => {
  ...
  describe('when custom icon', () => {
    if (isReact) {
      it('should pass props to render function', () => {
        const props = { children: cy.spy(), max: 5, size: BsRatingButtonSize.base };
        initializeComponent(props);
