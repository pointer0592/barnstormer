import { type ScrollableOptions, type BsScrollableOnScrollData, BsScrollableDirection } from './types';
...
export default class Scrollable {
  ...
  constructor(container: Element, ScrollableOptions?: Partial<ScrollableOptions>) {
    if (!(container instanceof HTMLElement))
      throw new Error(`BsScrollable: Container is not a HTMLElement! Received: ${container}`);
  ...
  public prev(): void {
    ...
    if (options.direction === BsScrollableDirection.vertical)
  ...
  public scrollToIndex(index: number): void {
    ...
    if (children[index]) {
      ...
      if (this.options.direction === BsScrollableDirection.vertical) {
  ...
  public refresh(callback?: (data: BsScrollableOnScrollData) => void) {
    if (callback) {
  ...
  private calculate(): BsScrollableOnScrollData {
    return {
