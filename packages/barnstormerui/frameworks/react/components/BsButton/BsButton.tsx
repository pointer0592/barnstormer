...
import { type BsButtonProps, BsButtonSize, BsButtonVariant, polymorphicForwardRef } from '@barnstormer/react';
...
const getSizeClasses = (size: BsButtonProps['size'], square: BsButtonProps['square']) => {
  switch (size) {
    case BsButtonSize.sm:
      return [square ? 'p-1.5' : 'leading-5 text-sm py-1.5 px-3', 'gap-1.5'];
    case BsButtonSize.lg:
...
const BsButton = polymorphicForwardRef<typeof defaultButtonTag, BsButtonProps>((props, ref) => {
  const {
    as,
    className,
