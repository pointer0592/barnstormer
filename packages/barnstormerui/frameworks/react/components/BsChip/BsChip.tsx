...
import { BsChipProps, BsChipSize } from '@barnstormer/react';

const paddingForSize = (size: `${BsChipSize}`, square: boolean, slotPrefix: ReactNode, slotSuffix: ReactNode) => {
  switch (size) {
    case BsChipSize.sm:
...
const getSizeClasses = (size: BsChipProps['size']) => {
  switch (size) {
    case BsChipSize.sm:
...
const BsChip = forwardRef<HTMLInputElement, BsChipProps>(
  (
    {
      children,
