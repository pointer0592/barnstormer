...
import {
  composeHandlers,
  createPropsGetter,
  BsPopoverBasePlacement,
  usePopover,
  useDisclosure,
} from '@barnstormer/react';
import type { UseTooltipOptions } from '@barnstormer/react';
...
export function useTooltip(options?: UseTooltipOptions) {
  ...
  function arrowStyle() {
    if (middlewareData.arrow) {
      ...
      const basePlacement = placement.split('-')[0] as BsPopoverBasePlacement<typeof placement>;
      return {
        position: 'absolute',
        width: arrowSize,
        height: arrowSize,
